<template>
    <div class="deployment-form">
      ...
      <div>
        <label>Name:</label>
        <input type="text" v-model="deploymentRequest.object_meta.name" placeholder="Name (ex: my-pod)" />
      </div>
  
      <div>
        <label>Namespace (optional):</label>
        <input type="text" v-model="deploymentRequest.object_meta.namespace" placeholder="Namespace (default value: default)" />
      </div>
  
      <!-- Spec - Containers Inputs -->
      <div>
        <h3>Containers</h3>
        <div v-for="(container, index) in deploymentRequest.spec.containers" :key="index" class="container-form">
  ...
  export default {
    ...
    methods: {
      addContainer() {
        // Add new container
        this.deploymentRequest.spec.containers.push({ name: '', image: '' });
      },
      removeContainer(index) {
        // Delete specify container
        this.deploymentRequest.spec.containers.splice(index, 1);
      },
      async sendJson() {
        // Send JSON to /pods endpoint
        try {
          console.log(this.deploymentRequest)
          const response = await axios.post('/pods', this.deploymentRequest, {
            headers: {
